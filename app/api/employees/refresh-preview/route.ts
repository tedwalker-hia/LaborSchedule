...
import { RefreshPreviewBodySchema } from '@/lib/schemas/employee';
...
export async function POST(request: NextRequest) {
  ...
  const parsed = RefreshPreviewBodySchema.safeParse(await request.json());
  if (!parsed.success) {
  ...
  try {
    ...
    const result = await svc.previewRefresh(parsed.data);
