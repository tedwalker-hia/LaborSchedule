...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const empCode of employeeCodes) {
      ...
      let positionHistories = await getEmployeeHistoryByPosition(usrSystemCompanyId, empCode)
      ...
      for (const date of dates) {
        ...
        if (isMultiPosition) {
          ...
          const totalWeeklyHours = positionHistories.reduce(
            (sum, h) => sum + h.avgWeeklyHours,
            0,
          )
